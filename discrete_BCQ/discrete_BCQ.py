...
class discrete_BCQ(object):
    ...
    def train(self, replay_buffer):
        ...
        if self.using_reward_shaping:
            reward_shaping = self.discount * self.shaping_function.Q_target(
                next_state).max(1, keepdim=True)[0] - \
                             self.shaping_function.Q_target(state).max(1, keepdim=True)[0]
            reward = reward + self.phi * reward_shaping
